...
import com.sgs.citytax.databinding.ItemRopTaxReceiptBinding
import com.sgs.citytax.model.LicenseRenewalNoticeReceiptDetails
import com.sgs.citytax.model.TaxReceiptsDetails
import com.sgs.citytax.util.*
import java.util.*

class ROPTaxReceiptViewHolder(val mBinding: ItemRopTaxReceiptBinding) : RecyclerView.ViewHolder(mBinding.root) {

    fun bind(receiptDetails: TaxReceiptsResponse, iClickListener: IClickListener?) {

        bindPDO(receiptDetails.taxReceiptsDetails[0],receiptDetails.receiptDetails[0],receiptDetails)
    ...
    private fun bindPDO(
        ...
        taxReceiptsDetails: LicenseRenewalNoticeReceiptDetails,
        ...
        val collectBy = mBinding.txtCollectedByLabel.context.getString(R.string.rop_collected_by)
 ...
 //                   mBinding.txtPrintCounts.text = ""//TODO on requirement of Onsite Team, hiding duplicate print counts
            ...
            // region Owner's Name
            mBinding.llBusinessOwner.visibility = View.GONE
            ...
            // region Owner's Phone
            mBinding.llBusinessPhone.visibility = View.GONE
            receiptDetails.businessOwnerPhone?.let {
                var value = it
                if (it.contains(";")) value = it.replace(";", "\n")
                mBinding.txtBusinessPhone.text = value
                mBinding.llBusinessPhone.visibility = View.VISIBLE
            }
            // endregion
            ...
            // region Nature Of Occupancy
            receiptDetails.taxSubType?.let {
            ...
            taxReceiptsDetails?.let {
                it.BillingCycleCode?.let {
                    if (it=="DAYS")
                    {
                        mBinding.txtBusinessOwnerLabel.text = getString(R.string.merchant_name)
                        mBinding.txtBusinessPhoneLabel.text= getString(R.string.merchant_phone)
                        mBinding.llBusinessName.visibility=View.GONE
                        mBinding.llNotDialy.visibility=View.GONE
                        mBinding.llTotalAmountOfRar.visibility=View.GONE
                        mBinding.llTotalDeposits.visibility=View.GONE
                        mBinding.llRmainingPay.visibility=View.GONE

                    }
                    else
                    {
                        mBinding.txtBusinessOwnerLabel.text = getString(R.string.rop_business_owner_name)
                        mBinding.txtBusinessPhoneLabel.text= getString(R.string.receipt_business_owner_phone)
                        mBinding.llBusinessName.visibility=View.VISIBLE
                        mBinding.llNotDialy.visibility=View.GONE
                        mBinding.llTotalAmountOfRar.visibility=View.VISIBLE
                        mBinding.llTotalDeposits.visibility=View.GONE
                        mBinding.llBusinessPhone.visibility = View.GONE
                ...
                mBinding.llMarketName.visibility = View.GONE
                it.Market?.let {
                    mBinding.txtMarketName.text = it
                    mBinding.llMarketName.visibility = View.VISIBLE
                }
            }
            ...
            /*     mBinding.txtDateTimeStamp.text = formatDisplayDateTimeInMillisecond(Date())

                 receiptDetails.note?.let {
                     mBinding.txtTaxNoticeNote.text = HtmlCompat.fromHtml(it, 0)
                 }*/

         /*   if (receiptDetails.paymentMode?.toUpperCase(Locale.getDefault()) ==  Constant.PaymentMode.CHEQUE.toString()){
                mBinding.txtNoteLable.visibility = View.VISIBLE
                mBinding.txtTaxNoticeNote.visibility = View.VISIBLE
                receiptDetails.chequeNote?.let {
                    mBinding.txtTaxNoticeNote.text = it
