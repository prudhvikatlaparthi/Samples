...
import com.sgs.citytax.databinding.FragmentAssetPostCheckListSummaryBinding
...
class AssetPostCheckListSummaryFragment : BaseFragment() {
    private lateinit var mBinding: FragmentAssetPostCheckListSummaryBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        mBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_asset_post_check_list_summary, container, false)
        initComponents()
    ...
    private fun bindData() {
        ...
        APICall.getPostCheckListSummaryData(assetRentID, object : ConnectionCallBack<AssetPrePostCheckListSummaryResponse> {
            override fun onSuccess(response: AssetPrePostCheckListSummaryResponse) {
                mListener?.dismissDialog()
                if (response.prePostCheckListData != null && response.prePostCheckListData!!.isNotEmpty()) {
                    bindPostCheckListData(response.prePostCheckListData!!)
                } else {
    ...
    private fun bindPostCheckListData(assetRentalSpecifications: ArrayList<AssetRentalSpecificationsList>) {
        for (assetRentalSpecification in assetRentalSpecifications) {
            ...
            mBinding.llDynamicPostCheckListData.addView(linearLayout)
    ...
    private fun navigateToReceiptsScreen(assetRentId: Int) {
        ...
        intent.putExtra(Constant.KEY_TAX_RULE_BOOK_CODE, Constant.TaxRuleBook.ASSET_RETURN.Code)
