...
class PropertyLandTaxDetailAdapter(val listener: DetailsListener, private val mCode: Constant.QuickMenu) : RecyclerView.Adapter<PropertyLandTaxDetailAdapter.ViewHolder>() {
    ...
    class ViewHolder(var binding: RowTaxDetailBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(taxDetails: SAL_TaxDetails, listener: DetailsListener, code: Constant.QuickMenu) {
            ...
            if (code == Constant.QuickMenu.QUICK_MENU_PROPERTY_TAX_NOTICE || code == Constant.QuickMenu.QUICK_MENU_LAND_TAX_NOTICE)  {
            ...
            if (code == Constant.QuickMenu.QUICK_MENU_PROPERTY_TAX_COLLECTION || code == Constant.QuickMenu.QUICK_MENU_LAND_TAX_COLLECTION) {
                binding.btnPaymentHistory.visibility = VISIBLE
                if (taxDetails.TotalDue.toDouble() > 0 && taxDetails.currentTaxInvoiceNo>0)
            ...
            binding.btnCollect.setOnClickListener (object: OnSingleClickListener() {
