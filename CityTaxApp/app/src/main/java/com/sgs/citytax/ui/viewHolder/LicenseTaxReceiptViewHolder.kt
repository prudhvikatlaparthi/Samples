...
import com.sgs.citytax.databinding.ItemLicenseTaxReceiptBinding
import com.sgs.citytax.model.LicenseRenewalNoticeReceiptDetails
import com.sgs.citytax.model.TaxReceiptsDetails
import com.sgs.citytax.util.*
import java.util.*

class LicenseTaxReceiptViewHolder(val mBinding: ItemLicenseTaxReceiptBinding) : RecyclerView.ViewHolder(mBinding.root) {
    fun bind(taxReceiptsResponse: TaxReceiptsResponse, iClickListener: IClickListener) {

        bindCPReceiptDetails(taxReceiptsResponse.taxReceiptsDetails[0],taxReceiptsResponse.receiptDetails[0],taxReceiptsResponse)
    ...
    private fun bindCPReceiptDetails(
        ...
        mtaxReceiptsDetails: LicenseRenewalNoticeReceiptDetails?,
...
//                    mBinding.txtPrintCounts.text = ""//TODO on requirement of Onsite Team, hiding duplicate print counts
            ...
            taxReceiptsDetails.advanceReceivedID?.let {
                mBinding.imgQRCode.setImageBitmap(bindQRCode(Constant.ReceiptType.TAX_RECEIPT, it.toString(), taxReceiptsDetails.sycoTaxId
                        ?: "",taxReceiptsDetails.referanceNo))
            }
            ...
            taxReceiptsDetails.taxSubType?.let {
                mBinding.txtLicenseCategory.text = it
            }

            mtaxReceiptsDetails?.authorizedBeverages?.let {
