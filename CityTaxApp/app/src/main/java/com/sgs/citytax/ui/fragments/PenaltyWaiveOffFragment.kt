...
import com.sgs.citytax.api.response.GetIndividualTaxInvoicePenalties
import com.sgs.citytax.api.response.GetSearchIndividualTaxDetails
import com.sgs.citytax.api.response.PenaltyList
import com.sgs.citytax.api.response.PenaltyResponse
import com.sgs.citytax.databinding.FragmentPenaltyWaiveOffBinding
...
import com.sgs.citytax.ui.adapter.PenaltyWaiveOffAdapter
...
class PenaltyWaiveOffFragment : BaseFragment(), View.OnClickListener, IClickListener {

    private lateinit var mBinding: FragmentPenaltyWaiveOffBinding
    private var mListener: Listener? = null
    private var accountId = ""
    var pageIndex: Int = 1
    val pageSize: Int = 100
    private var mServiceRequest: InvoicePenalties? = null
    ...
    companion object {
        @JvmStatic
        fun newInstance(item: InvoicePenalties) = PenaltyWaiveOffFragment().apply {
    ...
    override fun initComponents() {
        arguments?.let {
            ...
            if (it.containsKey(Constant.KEY_PROPERTY_TAX_PENALTY_WAIVE_OFF))
                mVuComProperties = arguments?.getParcelable(Constant.KEY_PROPERTY_TAX_PENALTY_WAIVE_OFF)
    ...
    private fun initViews() {
        mBinding.expandableListView.setAdapter(PenaltyWaiveOffAdapter(this))
    }
    ...
    private fun bindData() {
        when (fromScreen) {
            Constant.QuickMenu.QUICK_MENU_INDIVIDUAL_TAX_PENALTY_WAIVE_OFF -> {
                ...
                val getIndividualTaxInvoicePenalties = GetIndividualTaxInvoicePenalties()
                getIndividualTaxInvoicePenalties.accountID = getSearchIndividualTaxDetails?.accountID?.toInt()
                getIndividualTaxInvoicePenalties.productCode = getSearchIndividualTaxDetails?.productCode
                getIndividualTaxInvoicePenalties.voucherNo = getSearchIndividualTaxDetails?.voucherNo?.toInt()
                APICall.individualTaxInvoicePenalties(getIndividualTaxInvoicePenalties, object : ConnectionCallBack<PenaltyList> {
                    override fun onSuccess(response: PenaltyList) {
                        penaltyList = response.Penalties.groupBy { it.taxInvoiceID!! } as HashMap<Int, ArrayList<InvoicePenalties>>
                        groupList = arrayListOf()

                        for (group in penaltyList!!) {
                            groupList?.add(group.key)
                        }

                        if (response.Penalties.size > 0)
                            (mBinding.expandableListView.expandableListAdapter as PenaltyWaiveOffAdapter)
                                    .set(groupList!!, penaltyList!!)
                        else (mBinding.expandableListView.expandableListAdapter as PenaltyWaiveOffAdapter)
                                .set(arrayListOf(), hashMapOf())

                        repeat(penaltyList!!.count()) {
            ...
            Constant.QuickMenu.QUICK_MENU_LAND_TAX_PENALTY_WAIVE_OFF -> {
                mListener?.showProgressDialog(getString(R.string.msg_please_wait))
                val getPropertyTaxInvoicePenalties = GetPropertyTaxInvoicePenalties()
                getPropertyTaxInvoicePenalties.productCode = mVuComProperties?.productCode
                getPropertyTaxInvoicePenalties.voucherNo = mVuComProperties?.propertyID
                APICall.getPropertyTaxInvoicePenalties(getPropertyTaxInvoicePenalties, object : ConnectionCallBack<PenaltyList> {
                    override fun onSuccess(response: PenaltyList) {
                        penaltyList = response.Penalties.groupBy { it.taxInvoiceID!! } as HashMap<Int, ArrayList<InvoicePenalties>>
                        groupList = arrayListOf()
                ...
                val searchFilter = PenalitySearchFilter()
                ...
                filterColumn.columnName = "CurrentDue"
                filterColumn.columnValue = "0"
                filterColumn.srchType = "greaterthan"
                listFilterColumn.add(listFilterColumn.size, filterColumn)

                filterColumn = FilterColumn()
                filterColumn.columnName = "PenaltyAmount"
                filterColumn.columnValue = "0"
                filterColumn.srchType = "greaterthan"
                listFilterColumn.add(listFilterColumn.size, filterColumn)
                ...
                tableDetails.tableOrViewName = "VU_SAL_TaxInvoicePenalties"
                tableDetails.primaryKeyColumnName = "PenaltyID"
                tableDetails.selectColoumns = "NoticeReferenceNo,CurrentDue,PenaltyID,PenaltyDate,PenaltyAmount,TaxInvoiceID"
                tableDetails.TableCondition = "AND"
                searchFilter.tableDetails = tableDetails


                APICall.getPenaltyList(searchFilter, object : ConnectionCallBack<PenaltyResponse> {
                    override fun onSuccess(response: PenaltyResponse) {
                        penaltyList = response.results.Penalties.groupBy { it.taxInvoiceID!! } as HashMap<Int, ArrayList<InvoicePenalties>>
                        groupList = arrayListOf()
                        ...
                        if (response.results.Penalties.size > 0)
                            (mBinding.expandableListView.expandableListAdapter as PenaltyWaiveOffAdapter)
                                    .set(groupList!!, penaltyList!!)
                        else (mBinding.expandableListView.expandableListAdapter as PenaltyWaiveOffAdapter)
                                .set(arrayListOf(), hashMapOf())
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        mBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_penalty_waive_off, container, false)
        initComponents()
    ...
    private fun showWaiveOffDialog(invoicePenalties: InvoicePenalties, isChild: Boolean, _totalDue: BigDecimal = BigDecimal.ZERO) {
        if (dialog != null && dialog?.isShowing!!)
            return
        dialog = Dialog(requireContext()) // removed applied style
    ...
    private fun validateView(isChild: Boolean, totalDue: BigDecimal?): Boolean {
        ...
        if (!isChild && rbPartial?.isChecked!! && edtPercentage?.text?.isEmpty()!! && edtAmount?.text.toString().toBigDecimal() == BigDecimal(0)) {
            Toast.makeText(requireContext(), getString(R.string.msg_zero_amount), Toast.LENGTH_SHORT).show()
            return false
        }
    ...
    private fun waiveOff(invoicePenalties: InvoicePenalties, isChild: Boolean, totalDue: BigDecimal?) {
        mListener?.showProgressDialog(getString(R.string.msg_please_wait))
        ...
        view.let {
            when (view.id) {
                R.id.btnGroupWaiveOff -> {
                    val invoicePenalties = obj as InvoicePenalties
                    var totalDue = BigDecimal.ZERO
                    for (item in penaltyList?.get(groupList?.get(position))!!) {
