...
class BusinessTransactionHistoryAdapter(val listener: IClickListener) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var mTransactions: ArrayList<BusinessTransaction> = arrayListOf()
    private val mItem = 0
    ...
    class LoadingViewHolder(var binding: ItemProgressBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(transaction: BusinessTransaction, itemClickListener: IClickListener, position: Int) {
    ...
    fun add(transaction: BusinessTransaction) {
        mTransactions.add(transaction)
        notifyItemInserted(mTransactions.size - 1)
    }

    fun addAll(transactions: List<BusinessTransaction>) {
        for (transaction in transactions) {
            add(transaction)
        }
    }

    fun remove(transaction: BusinessTransaction?) {
