...
import com.sgs.citytax.databinding.FragmentLawPenaltyWaiveOffBinding
...
import com.sgs.citytax.ui.adapter.LawPenaltyWaiveOffAdapter
import com.sgs.citytax.util.*
import kotlinx.android.synthetic.main.fragment_penalty_waive_off.*
...
class LawPenaltyWaiveOffFragment : BaseFragment(), View.OnClickListener, IClickListener {

    private lateinit var mBinding: FragmentLawPenaltyWaiveOffBinding
    private var mListener: Listener? = null
    private var accountId = ""
    var pageIndex: Int = 1
    val pageSize: Int = 100
    private var mServiceRequest: LawPenalties? = null
    ...
    var mLawPenalties: ArrayList<LawPenalties>? = null
    var lawPenaltyList: HashMap<Int, ArrayList<LawPenalties>>? = null

    companion object {
        @JvmStatic
        fun newInstance(item: LawPenalties) = LawPenaltyWaiveOffFragment().apply {
    ...
    override fun initComponents() {
        arguments?.let {
            ...
            if (it.containsKey(Constant.KEY_LAW_TAX_DETAILS))
                mLawPenalties = arguments?.getParcelableArrayList(Constant.KEY_LAW_TAX_DETAILS)
    ...
    private fun initViews() {
        mBinding.expandableListView.setAdapter(LawPenaltyWaiveOffAdapter(this))
    }

    private fun bindData() {
        when (fromScreen) {
            Constant.QuickMenu.QUICK_MENU_LAW_PENALTY_WAIVE_OFF -> {

                mLawPenalties?.let {
                    lawPenaltyList = mLawPenalties?.groupBy { it.taxInvoiceID!! } as HashMap<Int, ArrayList<LawPenalties>>
                    groupList = arrayListOf()

                    for (group in lawPenaltyList!!) {
                        groupList?.add(group.key)
                    }

                    if (it.size > 0)
                        (mBinding.expandableListView.expandableListAdapter as LawPenaltyWaiveOffAdapter)
                                .set(groupList!!, lawPenaltyList!!)
                    else (mBinding.expandableListView.expandableListAdapter as LawPenaltyWaiveOffAdapter)
                            .set(arrayListOf(), hashMapOf())

                    repeat(lawPenaltyList!!.count()) {
    ...
    private fun getLawPenaltyTransactions(code: String?, data: String, responseID:Int) {
        ...
        APICall.getLawPenaltyTransactions(getPenaltyTransactions, object : ConnectionCallBack<GetLawPenaltyTransactionsResponse> {
            override fun onSuccess(response: GetLawPenaltyTransactionsResponse) {
                ...
                if (response.Penalties != null && response.Penalties.isNotEmpty()) {
                    mLawPenalties=response.Penalties
                } else {
                    mLawPenalties?.clear()
                }
                bindData()
                navigateToPreviewScreen(responseID)
            }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        mBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_law_penalty_waive_off, container, false)
        initComponents()
    ...
    private fun showWaiveOffDialog(invoicePenalties: LawPenalties, isChild: Boolean, _totalDue: BigDecimal = BigDecimal.ZERO) {
    ...
    private fun waiveOff(invoicePenalties: LawPenalties, isChild: Boolean, totalDue: BigDecimal?) {
        ...
        APICall.penaltyWaiveOff(penaltyWaiveOff, object : ConnectionCallBack<Int> {
            override fun onSuccess(response: Int) {
                ...
                getLawPenaltyTransactions("TaxNoticeNo", mLawPenalties?.get(0)?.noticeReferenceNo!!,response)
        ...
        view.let {
            when (view.id) {
                R.id.btnGroupWaiveOff -> {
                    val invoicePenalties = obj as LawPenalties
                    var totalDue = BigDecimal.ZERO
                    for (item in lawPenaltyList?.get(groupList?.get(position))!!) {
